...
1. Boot from TF/SD card with ramdisk
  ...
  - Copy MLO, u-boot.img, uEnv_ramdisk.txt, zImage, myd_j335x.dtb, ramdisk.gz and images directory to TF/SD card.
...
5. Boot from Nand
  - Copy MLO, u-boot.img, uEnv_ramdisk.txt, zImage, myd_j335x.dtb, ramdisk.gz, rootfs.ubi and images directory to TF/SD card.
  - Rename uEnv_ramdisk.txt to uEnv.txt
  - Insert the TF/SD card to slot, and set bootmode to mmc0
...
6. Boot from EMMC
  - Format TF/SD with fat/fat32 format
  - Copy all images file compiling with "myd_j335x_emmc_defconfig" to the TF/SD card.
