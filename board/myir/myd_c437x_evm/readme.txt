...
Introduction
==============
To be able to use MYD-C437X-EVM board of MYIR with the images generated by
...
1. Boot from TF/SD card with ramdisk
  ...
  - Copy MLO, u-boot.img, uEnv_ramdisk.txt, zImage, myd_c437x_evm.dtb, myd_c437x_evm_hdmi.dtb, ramdisk.gz to TF/SD card.
  - Rename uEnv_ramdisk.txt to uEnv.txt
  - Insert the TF/SD card to TF slot, and set bootmode to mmc0
...
3. Boot from Ethernet with NFS
  ...
  - setenv ipaddr 192.168.1.100 ( set the ipaddr of myd_c437x_evm board)
...
4. Boot from Ethernet with ramdisk
  ...
  - tftpboot ${fdtaddr} myd_c437x_evm.dtb  ( myd_c437x_evm.dtb was copied to tftp server root directory)
