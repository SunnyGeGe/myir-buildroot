...
Introduction
==============
To be able to use MYIR RICOBOARD of MYIR with the images generated by
...
1. Boot from TF/SD card with ramdisk
  ...
  - Copy MLO, u-boot.img, uEnv_ramdisk.txt, zImage, myir_ricoboard.dtb, myir_ricoboard_hdmi.dtb, ramdisk.gz to TF/SD card.
...
4. Boot from Ethernet with ramdisk
  ...
  - tftpboot ${fdtaddr} myir_ricoboard.dtb  ( myir_ricoboard.dtb was copied to tftp server root directory)
